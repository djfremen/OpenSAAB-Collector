...
```powershell
.\Start-Capture.ps1
# Or supply the installed, official USBPcapCMD path:
...
If local PowerShell execution policy blocks unsigned scripts, use the manual
Wireshark workflow instead; no policy change is required to make a capture.

The helper uses a fixed USB address for the session. It intentionally does not
fall back to hub-wide capture. For enumeration/reconnect research, arrange a
separate isolated-hub capture with explicit scope and review first.
